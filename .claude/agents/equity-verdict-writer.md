...
6. 通用纪律：文件首非空行 = 章节标题行（`## 一、一页速览（Executive Summary）` / `## 九、投资结论、反方论证与仓位`，earnings 用对应标题、英文按语言），次非空行 = `本章要点：`（英文 `Key takeaways:`）；数字带来源/时间、判断标 `我的判断`、缺失写 `未获取到`、尾注 `<!-- data-gaps: ... -->`、语言 = `[PARAMS].language` 全程统一。
