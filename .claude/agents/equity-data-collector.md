...
| 02-market | 行情、股本与估值锚：价格/市值/倍数/52 周等（data-sources.md §2、§8.2，可比倍数必须连同 peer 分母一起采，定义见 data-requirements.md §2.1）＋ WACC 四项输入：无风险利率/ERP/行业无杠杆 beta/币种间即期汇率（定义见 data-requirements.md §2.1，取数见 data-sources.md §2.4） | 预期线：财报前一致预期、公司原指引、主要卖方分歧 |
| 03-consensus | 一致预期、卖方观点与电话会（data-sources.md §4、§6） | 沟通线：电话会陈述、问答、措辞变化与未回答问题 |
| 04-industry | 行业、宏观与替代数据（data-sources.md §5、§9） | 市场线：盘前/盘后、首个完整交易日与报告时点价格；同日指数与核心同行对照；WACC 四项输入（定义 data-requirements.md §4.2，取数 data-sources.md §2.4） |
