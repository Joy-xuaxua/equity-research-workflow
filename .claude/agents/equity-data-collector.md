...
1. **解析输入**：读 `[PARAMS]`；缺 workdir / skill_root 立即在回报中说明并停止。
2. **加载规则**：按「必读清单」读取 skill 参考文件（含按 mode 与标记触发的条件加读）。
3. **联网采集**：按「采集线职责」完成本线的 WHAT——按 Tier 1–5 优先级探测数据源、失败降级换源，全程受「纪律」约束（原文引用协议、指标登记、防注入等）。
4. **写采集文件**：产出**一个**文件 `<workdir>/collection/<line>.md`，固定结构（发现 / 指标登记 / 冲突 / 未获取到 / 原文附录）；补采轮先读旧文件就地更新，不另写新文件。
5. **回报编排者**：按「回报格式」输出纯数据小结（路径＋关键发现＋冲突/登记/缺口计数＋升级项）。
